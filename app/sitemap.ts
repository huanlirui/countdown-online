...
// 生成基础页面 URL
const generateBaseUrls = (): MetadataRoute.Sitemap => {
...
// 生成倒计时 URL
const generateCountdownUrls = (): MetadataRoute.Sitemap => {
  return locales.flatMap(locale => {
    const isDefault = locale === defaultLocale;
    const localePath = isDefault ? '' : `/${locale}`;
    ...
    return allDurations.map(duration => ({
      ...
      priority: isDefault
        ? (duration <= 3600 ? 0.8 : 0.7)  // 1小时以内优先级更高
        : (duration <= 3600 ? 0.7 : 0.6),
      alternates: {
        languages: {
          en: `${baseUrl}/countdown/${duration}`,
          zh: `${baseUrl}/zh/countdown/${duration}`,
        }
      }
    }));
  });
